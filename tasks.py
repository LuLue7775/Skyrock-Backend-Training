...
def set_project(ctx, config):
    ...
    ctx.run('gcloud config set project {project}'.format(
        project=config_dict['project']), echo=True)
...
def upload_secrets(ctx, config):
    ...
    env_path = os.path.join(config_dict['env_dir'], '.env')
    ...
    ctx.run(
        'kubectl create secret generic {project_name} --from-env-file {env_path}'.format(
            project_name=settings_dict['project_name'],
            env_path=env_path))
...
def create_bucket(ctx, config, bucket_name):
    ...
    ctx.run(
        'gsutil mb gs://{bucket_name}'.format(bucket_name=bucket_name), echo=True)
